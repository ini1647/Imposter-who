...
export default function ResultsScreen({ route, navigation }) {
  ...
  useEffect(()=> {
    async function load() {
      ...
      const list = Object.keys(psObj).map(k=>psObj[k]);
      setPlayers(list);
  ...
  function tallyVotes() {
    ...
    const votedId = Object.keys(counts).find(id => counts[id] === max);
    return players.find(p=>p.id===votedId);
  }

  async function updateScores(imposterCaught, impGuessedCorrectly) {
    for (const p of players) {
      const path = `rooms/${code}/players/${p.id}`;
      const newScore = (p.score || 0) + (p.role === 'imposter' ? (imposterCaught ? -1 : impGuessedCorrectly ? 2 : 1) : (imposterCaught ? 1 : -1));
      await update(ref(db, path), { score: newScore });
    }
  }
